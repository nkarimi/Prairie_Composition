---
title: "00c.calculate.PD.metrics"
...
#September 2019 moved from 00.load.and.format.cover to here
```{r}
###################################################################
...
###################################################################
#dat17 <- read.csv(file = '../DATA/2017_VegetationCover/cover.matrix.2017-10-06.csv', as.is = TRUE) #as_planted to obs_2017(dat) exclude spring: 105 species
...
```{r}
###################################################################
##CORRECT FOR SPRING SPECIES 
###################################################################
#covMat.NA <- covMat.NA[, !(colnames(covMat.NA) %in% spp.oddballs), drop=TRUE] 
#
...
spp.spring <- c('Maianthemum_racemosum',
                'Polygonatum_biflorum',
                'Dodecatheon_meadia',
                'Allium_canadense',
                'Allium_cernuum',
                'Camassia_scilloides')
#spp.oddballs <- unique(c(spp.didNotEstablishWell, spp.spring)) #127-105 = 22

#tr.pruned <- drop.tip(tr, which(tr$tip.label %in% spp.oddballs))
#colnames(covMat.0) -> spp.names #col names from 2017 matrix 105
#tr.pruned <- drop.tip(tr.pruned, which(!tr.pruned$tip.label %in% spp.names))
#tr.pruned <- drop.tip(tr, which(!tr$tip.label %in% spp.names))
```
...
#PD METRICS
###################################################################
#dat = 2017 taken from Hipp script
