...
```{r load data}
#source 00.load.for.analyses.Rmd: df.pa.plug, df.pa.seed 
...
DF1 <-as.matrix((subset(df.pa.plug, select = Agalinis_tenuifolia:Zizia_aurea)))
...
```{r plot plugPA}
year = as.factor(df.pa.plug$year)
plot = df.pa.plug$plot
NMDS.plug.PAcomp <- as.data.frame(scores(jmds)) #NMDS.plug.PAcomp
NMDS.plug.PAcomp <- cbind(NMDS.plug.PAcomp, year, plot, df.pa.plug$Code)
NMDS.plug.PAcomp$phyD <-  data.frame(phyD = substr(df.pa.plug$Code,1,1)) 
...
```{r}
nmds.plug.comp <- metaMDS(df.cover.plug[8:ncol(df.cover.plug)], k=3, maxit=20000, distance="bray", autotransform = FALSE, trymax=100)
nmds.plug.comp <- metaMDS(df.cover.plug[8:length(df.cover.plug)], k=3, maxit=20000, distance = 'bray', previous.best = nmds.plug.comp, autotransform = FALSE, trymax=100)
...
```{r}
#P/A Dissimilarity for seed and plug by year
df.pa.plug.yr <- split(df.pa.plug, df.pa.plug$year) #2016 - spring 2019
...
```{r}
plot_grid(PP, FigPlugPACompOrd, labels = "auto", nrow = 1) -> Figure4.PAComposition.Plug
Figure4.PAComposition.Plug
#ggsave(file="../OUT/Figure4.PAComposition.Plug.pdf")
```
