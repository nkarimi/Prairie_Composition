...
```
#SIMPER
##Contribution of individual species to the overall dissimilarity
Identify which species contribute to dissimilarity between groups
Similarity percentage, simper (Clarke 1993) is based on the decomposition of Bray-Curtis dissimilarity index (see vegdist, designdist). The contribution of individual species to the overall Bray-Curtis dissimilarity however "The method very badly confounds the mean between group differences and within group variation, and seems to single out variable species instead of distinctive species (Warton et al. 2012).Even if you make groups that are copies of each other, the method will single out species with high contribution, but these are not contributions to non-existing between-group differences but to within-group variation in species abundance."
```{r}
df.cover[,1:8] -> df.env #made in 00b.load.for.analyses
...
#sim is ordered by cusum not species; replace cumsum with ava = 2019.09 contribution (compared to 2017)
as.data.frame(contribution.value) -> contribution.value
names(contribution.value) <- c("species", "cumsum")
contribution.value.ordered <- contribution.value[order(contribution.value$species),] #reorder contribution.value alphabetically
...
test.traits <- traits.mat[1:14]
rownames(test.traits) <- test.traits$Species
Ord <- metaMDS(test.traits[-1], maxit=10000, k=3) #stress = 0.166411
...
m_com = as.matrix(pc)
#Now use the function “anosim” from the package vegan to test whether there is a statistical difference between groups. In the following code, ‘m_com’ is the community abundance matrix I am testing; ‘pc$Type’ is the column (“Type”) in my data frame (“pc”) that contains the grouping information
ano = anosim(m_com, pc2$simperType, distance = "bray", permutations = 9999)
ano
###After generating a cluster analysis object from hclust(), groups may be defined using cutree(), based on the clustering. A colour vector, with as many colours as groups, will be indexed by the cutree() object when plotting the points of a metaMDS() solution.
...
```{r}
#replaced Tch with change based on pct_change2 from winners.losers
...
simper.fig
#ggsave(simper.fig, file="../OUT/simper.figure_lines.png")
#ggsave(simper.fig, file="../OUT/simper.figure_lines.pdf")
```
